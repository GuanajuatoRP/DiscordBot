...
export class HelpCommand extends Command {
    ...
    execute(interaction : CommandInteraction) {
        ...
        const commands: Command[] = this.client.collections.commands.map(c => c[0]) as Command[] //Get All Commands loaded for the bot 
    ...
    onAutocomplete(interaction: AutocompleteInteraction) {
        ...
        const choices: Array<string> = (this.client.collections.commands.map(c => c[0]) as Command[]).filter(c => (c as Command).category != `InDev` && (c as Command).adminsOnly == false && (c as Command).type == 'SLASH_COMMAND').map(c => (c as Command).name)
