    ...
    execute(interaction) {
        ...
        if (interaction.options.get('add') == null && interaction.options.get('remove') == null) {
            ...
            interaction.guild.roles.cache.get(appConfig_json_1.default.Roles.ADMIN).members.forEach(u => {
                if (appConfig_json_1.default.botConfig.admins.includes(u.id) && u.user.username === interaction.options.get('remove').value || u.nickname === interaction.options.get('remove')) {
                    if (u.id === appConfig_json_1.default.botConfig.dercrakerId) {
