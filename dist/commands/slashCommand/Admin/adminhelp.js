...
class AdminHelpCommand extends sheweny_1.Command {
    ...
    execute(interaction) {
        ...
        if (!commandName.get('commande')) { //si aucune commande est donner en paramètre
            ...
            const command = this.client.collections.commands.map(c => c[0]).filter(c => c.name == CName)[0]; //récupération du nom 
    ...
    onAutocomplete(interaction) {
        ...
        const choices = this.client.collections.commands.map(c => c[0]).filter(c => c.category == `InDev` || c.category == `Admin` && c.adminsOnly === true && c.type === 'SLASH_COMMAND').map(c => c.name);
